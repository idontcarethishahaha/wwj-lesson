...
let loginForm = ref();
let loginFormData = reactive({username: 'admin', password: 'admin'});
 ...
 * 忘记密码
 *
 * 1. 使用 ElNotification 在右上角通知：“员工测试账号: admin / 123456789”
 */
function forgetPassword() {
  ElNotification.info({
    title: '通知列表',
    message: '测试账号: admin / admin',
    position: 'top-right',
  });
}
...
.login-body {
  height: 100vh; // 高度
  background: url("@/assets/image/loginBackground.jpg") no-repeat; // 背景图片（不平铺）
